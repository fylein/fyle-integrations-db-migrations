DROP VIEW if exists _export_log_fatal_in_progress_export_processed_tasks_view;

CREATE OR REPLACE VIEW _export_log_fatal_in_progress_export_processed_tasks_view AS
WITH export_log_fatal_in_progress_export_processed_tasks AS (
    SELECT COUNT(*) AS export_log_fatal_in_progress_export_processed_tasks_count
    FROM export_logs
    WHERE
        workspace_id IN (SELECT id FROM prod_workspaces_view)
        AND status IN ('FATAL', 'IN_PROGRESS', 'EXPORT_PROCESSED')
        AND updated_at < NOW() - INTERVAL '45 mins'
)
SELECT * FROM export_log_fatal_in_progress_export_processed_tasks;