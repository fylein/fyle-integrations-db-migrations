...
DROP TRIGGER IF EXISTS monitor_updates ON qbo_credentials;

DROP TRIGGER IF EXISTS monitor_deletes ON qbo_credentials;
CREATE TRIGGER monitor_deletes
AFTER DELETE ON qbo_credentials
FOR EACH ROW
...
DROP TRIGGER IF EXISTS monitor_deletes ON expenses;
